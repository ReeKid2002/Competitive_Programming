...
int Count_no_of_node_in_Binary_tree(BinaryTreeNode<int> *root){
    if(root==NULL)
        return 0;
    int total_no_of_node = 1;
    total_no_of_node+=Count_no_of_node_in_Binary_tree(root->left_child);
    total_no_of_node+=Count_no_of_node_in_Binary_tree(root->right_child);
    return total_no_of_node;
}
 int main(){
    BinaryTreeNode<int> *root = Input_into_binary_tree_using_level_order_insertion();
    Level_order_Display(root);
    int no_of_node = Count_no_of_node_in_Binary_tree(root);
    cout<<"Number of Node: "<<no_of_node;
 }
