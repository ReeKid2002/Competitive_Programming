...
TreeNode<int>* level_order_insert(){
    ...
    while(!queue_node.empty()){
        ...
        for(int i=0; i<no_of_child;i++){
            int child_data;
            cout<<"Enter Data for "<<front_node->data<<": ";
            cin>>child_data;
...
int num_of_node(TreeNode<int>* root){
    int no_of_node = 1;
    for(int i=0;i<root->child.size();i++){
        no_of_node+=num_of_node(root->child[i]);
    }
    return no_of_node;
}
...
int main(){
    ...
    int no_of_node = num_of_node(root);
    cout<<"Number of Node: "<<no_of_node<<endl;
}
